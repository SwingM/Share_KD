...
# 翻倒自动救援（仿真扶正式，见 fall_rescue.py）：
#   检测到翻倒(|roll|或|pitch|超阈值)并【持续 FALL_DWELL 真实秒】后，
#   在【当前 x,y 与朝向不变】下把狗扶正，再 Passive→GetUp→RL 自己站回来，任务继续。
#   计时用真实时钟(墙钟)：Gazebo RTF≈0.1，10 真实秒 ≈ 1 仿真秒，救援更及时。
#   关闭/调整: FALL_RESCUE=false FALL_DWELL=5 bash explore_three_dogs.sh
FALL_RESCUE=${FALL_RESCUE:-true}
FALL_DWELL=${FALL_DWELL:-10.0}
FALL_ROLL_DEG=${FALL_ROLL_DEG:-75.0}
FALL_PITCH_DEG=${FALL_PITCH_DEG:-75.0}
RESCUE_Z=${RESCUE_Z:-0.45}

# 目标检测：把物体位姿发到 SLAM 的 map 坐标系（默认关闭）：
#   开启: TARGET_SEARCH=true bash explore_three_dogs.sh
#   单进程同时订阅三狗 RGB-D，共享一份 YOLO 权重，只做感知不建图（栅格由 RTAB-Map 提供），
#   输出 /target_search/objects (PoseArray, frame=merged_map)
#        + /target_search/objects_class_ids / objects_confidence（与位姿一一对应）
#        + /target_search/objects_markers（RViz 可视化，带类别文字）
#   ⚠ 需要 ultralytics（pip install ultralytics），否则节点只告警不检测。
#   只检测指定类别: TARGET_CLASS=person TARGET_DETECT_ALL=false
TARGET_SEARCH=${TARGET_SEARCH:-false}
TARGET_CLASS=${TARGET_CLASS:-airplane}
TARGET_DETECT_ALL=${TARGET_DETECT_ALL:-true}
TARGET_CONF=${TARGET_CONF:-0.3}
TARGET_RATE=${TARGET_RATE:-5.0}

# 出生点默认取分区 json 里 box 的 world center（bbox_world.center_world），
# 每只狗直接落在它负责的 box 的几何中心。
# （换分区必须同步，否则狗出生在 box 外 -> 地图与 box 不重叠 -> 覆盖率恒为 0。
#   之前 go2_3 就是这么变成 0% 的）
# 兜底：旧 json 没有 center_world 时，用 corners_world 的 min/max 均值算。
...
try:
    ...
    for bid in ids:
        ...
        if not b:
            continue
        bb = b[0]["bbox_world"]
        c = bb.get("center_world")
        if not c:
            cs = bb["corners_world"]
            xs = [p[0] for p in cs]
            ys = [p[1] for p in cs]
            c = [(min(xs) + max(xs)) / 2.0, (min(ys) + max(ys)) / 2.0]
        out.append("%.2f,%.2f" % (c[0], c[1]))
...
pkill -9 -f "fall_rescue" 2>/dev/null
pkill -9 -f "multi_target_detector" 2>/dev/null; pkill -9 -f "target_detector" 2>/dev/null
...
# [7.8] 目标检测（可选，默认关闭；需要 ultralytics）
if [ "$TARGET_SEARCH" = "true" ]; then
    ROBOTS_CSV=""
    for i in "${DOGS[@]}"; do
        ROBOTS_CSV="${ROBOTS_CSV:+$ROBOTS_CSV,}go2_$i"
    done
    echo "[7.8/9] 启动目标检测 (robots=$ROBOTS_CSV, frame=merged_map)..."
    : > "$LOG/target_search.log"
    setsid bash -c "source /opt/ros/humble/setup.bash && source $W/install/setup.bash && \
        source $WS2/install/setup.bash && \
        FASTRTPS_DEFAULT_PROFILES_FILE=$SCRIPT_DIR/fastdds_profile.xml \
        exec ros2 launch target_search multi_target_search_launch.py \
            use_sim_time:=true robots:=$ROBOTS_CSV \
            target_class:=$TARGET_CLASS detect_all_classes:=$TARGET_DETECT_ALL \
            confidence_threshold:=$TARGET_CONF detection_rate:=$TARGET_RATE" \
        > "$LOG/target_search.log" 2>&1 < /dev/null &
    sleep 2
    echo "      target_search 已启动 (log: $LOG/target_search.log)"
    echo "      物体位姿 /target_search/objects (+ _class_ids/_confidence/_markers)"
    echo "      查看: rviz2 MarkerArray=/target_search/objects_markers，Fixed Frame=merged_map"
else
    echo "[7.8/9] 目标检测已关闭 (TARGET_SEARCH=false)"
fi
...
# [9.5] 翻倒自动救援监看（每只狗一个进程，独立于探索进程）
if [ "$FALL_RESCUE" = "true" ]; then
    echo "[9.5/9] 启动翻倒自动救援监看 (翻倒持续 ${FALL_DWELL}s 真实时钟后原地扶正)..."
    for i in "${DOGS[@]}"; do
        setsid bash -c "cd '$SCRIPT_DIR' && \
            FASTRTPS_DEFAULT_PROFILES_FILE='$SCRIPT_DIR/fastdds_profile.xml' \
            exec python3 fall_rescue.py \
            --robot go2_$i \
            --fifo '$LOG/keys_$i' \
            --model go2_${i}_gazebo \
            --fall-roll-deg $FALL_ROLL_DEG \
            --fall-pitch-deg $FALL_PITCH_DEG \
            --fall-dwell $FALL_DWELL \
            --rescue-z $RESCUE_Z" \
            > "$LOG/fall_rescue_$i.log" 2>&1 < /dev/null &
        echo "      go2_$i 救援监看已启动 (log: $LOG/fall_rescue_$i.log)"
    done
else
    echo "[9.5/9] 翻倒自动救援已关闭 (FALL_RESCUE=false)"
fi
...
while true; do
    ...
    for i in "${DOGS[@]}"; do
        ...
        RESCUE=$(grep -ao '\[RESCUE\].*' "$LOG/fall_rescue_$i.log" 2>/dev/null | tail -1)
        echo "  go2_$i: 第${CELL:-?}次选块(当前划分${PLAN:-?}块) 子区域=${CELL_COV:-N/A}% 整体=${OVERALL:-N/A}%"
        [ -n "$RESCUE" ] && echo "        救援: $RESCUE"
    done
    DONE=0
    for i in "${DOGS[@]}"; do
