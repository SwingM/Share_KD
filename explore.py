...
class Explorer(Node):
    def __init__(self):
        ...
        # ─── RViz 可视化发布 ────────────────
        self._frontier_pub = self.create_publisher(Marker, '/explorer/frontiers', 1)
        self._best_pub = self.create_publisher(Marker, '/explorer/best_frontier', 1)
        self._region_pub = self.create_publisher(Marker, '/explorer/region', 1)
    ...
    def get_robot_pose(self):
        try:
            t = self.tf_buffer.lookup_transform(
                'map', 'go2_1/base_link', rclpy.time.Time())
    ...
    def in_bounds(self, x, y):
        return BOUND_X_MIN <= x <= BOUND_X_MAX and BOUND_Y_MIN <= y <= BOUND_Y_MAX
    ...
    def coverage_ratio(self):
        if not self._map_ready:
            return 0.0
        x0, y0 = self.coord_to_index(BOUND_X_MIN, BOUND_Y_MIN)
        x1, y1 = self.coord_to_index(BOUND_X_MAX, BOUND_Y_MAX)
        known = total = 0
    ...
    def send_goal(self, x, y, yaw=0.0):
        ...
        self._goal_done = False
        self._last_status = None
        f = self._client.send_goal_async(g, feedback_callback=self.feedback_cb)
        f.add_done_callback(self.response_cb)
    ...
    def save_map(self):
        ...
        client = self.create_client(Empty, '/rtabmap/save')
        if client.wait_for_service(timeout_sec=5.0):
            future = client.call_async(Empty.Request())
            rclpy.spin_until_future_complete(self, future, timeout_sec=30.0)
            if future.result() is not None:
                self.get_logger().info('RTAB-Map database saved!')
            else:
                self.get_logger().warn('Save service call failed')
        else:
            self.get_logger().warn('/rtabmap/save service not available, saving via CLI...')
            import subprocess
            subprocess.run(['ros2', 'run', 'nav2_map_server', 'map_saver_cli',
                           '-f', 'explored_map', '--ros-args', '-p', 'save_map_timeout:=30'], check=False)

    # ─── 运 行 ────────────────────────────
    def run(self):
        self.get_logger().info('Waiting for /map (up to 180s)...')
        start = time.time()
...
def main():
    ...
    e.destroy_node()
    rclpy.shutdown()
    print(f'\n==== Exploration {"SUCCESS" if ok else "FAILED"} ====')
